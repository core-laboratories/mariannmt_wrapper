...
import 'mariannmt_bindings_generated.dart';

/// Marian NMT 翻译器异常
class MarianTranslationException implements Exception {
  final String message;
  final int? errorCode;

  MarianTranslationException(this.message, [this.errorCode]);

  @override
  String toString() =>
      'MarianTranslationException: $message${errorCode != null ? ' (code: $errorCode)' : ''}';
}
...
class _MarianBackground {
  static _MarianBackground? _instance;

  static _MarianBackground get instance {
    _instance ??= _MarianBackground._();
    return _instance!;
  }
    ...
    _starting = () async {
      ...
      _receiveSub ??= _receivePort!.listen((dynamic message) {
        ...
        if (ok) {
          ...
          completer.completeError(MarianTranslationException(err));
        }
      });
      ...
      _isolate = await Isolate.spawn<_IsolateInit>(
        ...
        debugName: 'mariannmt_wrapper_worker',
      );

      try {
        _sendPort = await handshake.future.timeout(const Duration(seconds: 5));
      } on TimeoutException {
        throw MarianTranslationException(
          'Failed to start Marian worker isolate (timeout)',
        );
      }
    }();
  ...
  Future<T> _call<T>(String cmd, Map<String, Object?> payload) async {
    ...
    _sendPort!.send(<String, Object?>{'id': id, 'cmd': cmd, ...payload});
  ...
  Future<List<String>> translateMultiple(List<String> inputs, String key) =>
      _call<List<String>>('translateMultiple', <String, Object?>{
        'inputs': inputs,
        'key': key,
      });

  Future<List<String>> pivotMultiple(
    List<String> inputs,
    String firstKey,
    String secondKey,
  ...
  Future<Map<String, Object?>> detectLanguage(String text, String? hint) =>
      _call<Map<String, Object?>>('detectLanguage', <String, Object?>{
        'text': text,
        'hint': hint,
      });

  Future<void> cleanup(bool resetService) =>
      _call<void>('cleanup', <String, Object?>{'resetService': resetService});

  void shutdown() {
    // 让所有未完成的请求尽快失败，避免退出时 await 永久悬挂。
    if (_pending.isNotEmpty) {
      final err = MarianTranslationException('Marian worker shutdown');
...
void _marianWorkerMain(_IsolateInit init) {
  ...
  port.listen((dynamic raw) {
    ...
    Map<String, Object?> err(Object error) => <String, Object?>{
      'id': id,
      'ok': false,
    ...
    try {
      switch (cmd) {
        case 'init':
          MarianTranslator.initializeService();
          mainSendPort.send(ok(null));
          return;
        case 'loadModel':
          MarianTranslator.loadModel(
            raw['cfg'] as String,
            raw['key'] as String,
          );
          mainSendPort.send(ok(null));
        ...
        case 'translateMultiple':
          ...
          final out = MarianTranslator.translateMultiple(inputs, key);
        ...
        case 'pivotMultiple':
          ...
          final out = MarianTranslator.pivotMultiple(
            inputs,
            firstKey,
            secondKey,
          );
        ...
        case 'detectLanguage':
          ...
          final res = MarianTranslator.detectLanguage(text, hint);
          mainSendPort.send(
            ok(<String, Object?>{
          ...
          return;
        case 'cleanup':
          MarianTranslator.cleanup(raw['resetService'] as bool? ?? false);
          mainSendPort.send(ok(null));
          return;
        default:
...
class MarianTranslator {
  static const String _libName = 'mariannmt_wrapper';

  /// 动态库实例
  static ffi.DynamicLibrary? _dylib;

  /// 绑定实例
  static MarianNativeBindings? _bindings;
  ...
  static void _ensureInitialized() {
    ...
    _bindings = MarianNativeBindings(_dylib!);
  }
  ...
  /// 必须在调用其他方法之前调用此方法。
  ///
  /// 抛出 [MarianTranslationException] 如果初始化失败。
  static void initializeService() {
    _ensureInitialized();
    final result = _bindings!.bergamot_initialize_service();
    if (result != 0) {
      throw MarianTranslationException('Failed to initialize service', result);
    }
  }
  ...
  static Future<void> initializeServiceAsync() {
    return _MarianBackground.instance.initializeService();
  }
  ...
  ///
  /// 抛出 [MarianTranslationException] 如果加载失败。
  static void loadModel(String cfg, String key) {
    ...
    try {
      ...
      if (e is MarianTranslationException) {
        rethrow;
      }
      throw MarianTranslationException(
        'Unexpected error loading model $key: $e',
      );
    } finally {
      malloc.free(cfgPtr);
      malloc.free(keyPtr);
  ...
  static Future<void> loadModelAsync(String cfg, String key) {
    return _MarianBackground.instance.loadModel(cfg, key);
  }
  ...
  /// 返回翻译结果列表，顺序与输入列表对应。
  ///
  /// 抛出 [MarianTranslationException] 如果翻译失败。
  static List<String> translateMultiple(List<String> inputs, String key) {
    ...
    try {
      ...
      if (result != 0) {
        throw MarianTranslationException('Failed to translate', result);
      }
  ...
  ///
  /// 推荐在 Flutter 场景使用：避免同步 FFI 阻塞 UI isolate。
  static Future<List<String>> translateMultipleAsync(
    List<String> inputs,
    String key,
  ) {
    return _MarianBackground.instance.translateMultiple(inputs, key);
  }
  ...
  static String translate(String input, String key) {
    ...
    if (results.isEmpty) {
      throw MarianTranslationException('Translation returned empty result');
    }
  ...
  static List<String> pivotMultiple(
    ...
    try {
      ...
      if (result != 0) {
        throw MarianTranslationException('Failed to pivot translate', result);
      }
  ...
  static Future<List<String>> pivotMultipleAsync(
    ...
    return _MarianBackground.instance.pivotMultiple(
      inputs,
      firstKey,
      secondKey,
    );
  ...
  static String pivot(String input, String firstKey, String secondKey) {
    ...
    if (results.isEmpty) {
      throw MarianTranslationException(
        'Pivot translation returned empty result',
      );
    }
    return results.first;
  }
  ...
  ///
  /// 抛出 [MarianTranslationException] 如果检测失败。
  static DetectionResult detectLanguage(String text, [String? hint]) {
    ...
    try {
      ...
      if (result != 0) {
        throw MarianTranslationException('Failed to detect language', result);
      }
  ...
  ///
  /// 推荐在 Flutter 场景使用：避免同步 FFI 阻塞 UI isolate。
  static Future<DetectionResult> detectLanguageAsync(
    String text, [
    String? hint,
  ]) async {
    final map = await _MarianBackground.instance.detectLanguage(text, hint);
  ...
  static void cleanup(bool resetService) {
    if (_bindings != null) {
      _bindings!.bergamot_cleanup(resetService ? 1 : 0);
    }
  }
  ...
  static Future<void> cleanupAsync(bool resetService) async {
    await _MarianBackground.instance.cleanup(resetService);
    _MarianBackground.instance.shutdown();
  }
