...
/// Bindings for `src/mariannmt_wrapper.h`.
///
/// Regenerate bindings with `dart run ffigen --config ffigen.yaml`.
///
class MarianNativeBindings {
  /// Holds the symbol lookup function.
  final ffi.Pointer<T> Function<T extends ffi.NativeType>(String symbolName)
  _lookup;

  /// The symbols are looked up in [dynamicLibrary].
  MarianNativeBindings(ffi.DynamicLibrary dynamicLibrary)
    : _lookup = dynamicLibrary.lookup;

  /// The symbols are looked up with [lookup].
  MarianNativeBindings.fromLookup(
    ffi.Pointer<T> Function<T extends ffi.NativeType>(String symbolName) lookup,
  ) : _lookup = lookup;

  /// 清理资源（释放所有模型和服务）
  void bergamot_cleanup(int reset_service) {
    return _bergamot_cleanup(reset_service);
  }
